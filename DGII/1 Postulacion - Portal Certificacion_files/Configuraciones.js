...
// var inputNombreComercial = document.getElementById(
//   "inputNombreComercialProveedor"
// );
...
// if (inputRncProveedorSoftware == "124005132") {
//   inputRazonSocial.readOnly = true;
//   inputNombreComercial.readOnly = true;
//   $("#lblRncProveedorSoftware").attr("hidden", true);
//   $("#AlertarSoftwareErrores").attr("hidden", true);
//   $("#lblRazonSocialProveedorSoftware").attr("hidden", true);
//   $("#AlertarSoftwareErrores").attr("hidden", true);
...
function Refresh() {
  window.resetId = 0;
  if (
    document.querySelector('meta[http-equiv="refresh"]') != undefined &&
    document.querySelector('meta[http-equiv="refresh"]').content.length !== 0
  ) {
    var timeoutPeriod = document.querySelector(
...
$("#nuevaClave, #repeticionClave").on("keyup", function () {
  ...
  if ($("#nuevaClave").val() != undefined && $("#nuevaClave").val() != "") {
    if (
      ...
      $("#nuevaClaveInvalida")
        .html("La contraseña no cumple con el estándar esperado.")
        .css("color", "red");
      ...
      $("#nuevaClaveInvalida").val("");
      $("#nuevaClaveInvalida")
        .html("La nueva contraseña no puede ser igual a la anterior")
        .css("color", "red");
      $("#nuevaClaveInvalida").css("display", "block");
      $("#nuevaClaveValida").css("display", "none");
      $("#repeticionClaveInValida").hide();
  ...
  if (
    $("#nuevaClave").val() != undefined &&
    $("#repeticionClave").val() != undefined &&
    $("#nuevaClave").val() != "" &&
    $("#repeticionClave").val() != ""
  ) {
    if ($("#nuevaClave").val() != $("#repeticionClave").val()) {
      ...
      $("#repeticionClaveInValida")
        .html(
          "La nueva contraseña y su repetición no coinciden, favor reintentarlo."
        )
        .css("color", "red");
      ...
      $("#repeticionClaveInValida").hide();
      $("#repeticionClaveValida")
        .html("La nueva contraseña y su repetición coinciden")
        .css("color", "green");
      $(".pwds").removeClass("is-invalid");
    }
  }
...
function ValidarMotivoRetorno(motivo) {
  ...
  if ((motivo.length < 50) | (motivo.length > 500)) {
    ...
    $("#txtMotivoR").addClass("invalid");
    $("#MensajeRespuestaR").text(
      "La Justificación debe de tener de 50 a 500 caracteres"
    );
    return false;
  }
  $("#txtMotivoR").removeClass("invalid");
...
async function PaginacionLoad(postulacionid, tipoproceso, numeropagina) {
  ...
  var _url = localUrl.replace(
    localUrl.split("/").pop(),
    "CallLogProcesosViewComponent"
  );
  ...
  await $.ajax({
    ...
    url: _url,
    data: {
      postId: postulacionid,
      tProceso: tipoproceso,
      numPag: numeropagina,
    },
...
$("#frmGuardarFacturaConsumoSimulacion").submit(async (e) => {
  ...
  var _url = localUrl.replace(
    localUrl.split("/").pop(),
    "GuardarFacturaConsumoSimulacion"
  );
...
$("#inputRncProveedor").change(async (e) => {
  ...
  debugger;
  if (
    rncProveedor == inputRncProveedor ||
    rncProveedor == "" ||
    rncProveedor.length < 9 ||
    rncProveedor.length > 11
  ) {
    Habilitar_DeshabilitarCamposProveedor(true);
    return;
  }
  ...
  var _url = localUrl.replace(
    localUrl.split("/").pop(),
    "FoundContribuyenteRegistroApim"
  );

  if (rncProveedor == null || rncProveedor == "") {
    spinner.hide();
    return;
  }
  ...
  await $.ajax({
    ...
    .done((resp) => {
      if (resp.data != null) {
        ...
        Habilitar_DeshabilitarCamposProveedor(true);
        $("#lblRncProveedorSoftware").text(
          "El RNC/Cédula no se encuentra registrado"
        );
      }
...
function habilitaralertaRepresentante(habilitar) {
  if (habilitar) {
    $("#lblRncRepresentante").attr("hidden", true);
    $("#AlertarRepresentanteErrores").attr("hidden", true);
    $("#lblnombreRepresentante").attr("hidden", true);
    $("#AlertarRepresentanteErrores").attr("hidden", true);
    $("#lblCorreoRepresentante").attr("hidden", true);
    $("#AlertarRepresentanteErrores").attr("hidden", true);
  } else {
    $("#lblRncRepresentante").attr("hidden", false);
    $("#AlertarRepresentanteErrores").attr("hidden", false);
    $("#lblnombreRepresentante").attr("hidden", false);
    $("#AlertarRepresentanteErrores").attr("hidden", false);
    $("#lblCorreoRepresentante").attr("hidden", false);
    $("#AlertarRepresentanteErrores").attr("hidden", false);
  }
}
...
function habilitaralertaSoftware(habilitar) {
  if (habilitar) {
    $("#lblNombreSoftware").attr("hidden", true);
    $("#AlertarSoftwareErrores").attr("hidden", true);
    $("#lblVersionSoftware").attr("hidden", true);
    $("#AlertarSoftwareErrores").attr("hidden", true);
    $("#lblRncProveedorSoftware").attr("hidden", true);
    $("#AlertarSoftwareErrores").attr("hidden", true);
    $("#lblRazonSocialProveedorSoftware").attr("hidden", true);
    ...
    $("#lblNombreSoftware").attr("hidden", false);
    $("#AlertarSoftwareErrores").attr("hidden", true);
    $("#lblVersionSoftware").attr("hidden", false);
    $("#AlertarSoftwareErrores").attr("hidden", true);
    $("#lblRncProveedorSoftware").attr("hidden", false);
    $("#AlertarSoftwareErrores").attr("hidden", false);
    $("#lblRazonSocialProveedorSoftware").attr("hidden", false);
    $("#AlertarSoftwareErrores").attr("hidden", false);
  }
}
...
function Habilitar_DeshabilitarCamposProveedor(isHabilitar) {
  ...
  if (inputRncProveedorSoftware.value == "124005132") {
    inputRazonSocial.readOnly = true;
    inputNombreComercial.readOnly = true;
    inputRazonSocial.value = "FORTECH SRL";
    inputNombreComercial.value = "FORTECH";
...
$("#btnGenerarArchivoValidaciones").click(async function (e) {
  ...
  if (nombreSoftware == null || nombreSoftware == "") {
    $("#lblNombreSoftware").attr("hidden", false);
    $("#AlertarSoftwareErrores").attr("hidden", true);
    isrequerido = true;
  }
  if (versionSoftware == null || versionSoftware == "") {
    $("#lblVersionSoftware").attr("hidden", false);
    $("#AlertarSoftwareErrores").attr("hidden", true);
    isrequerido = true;
  }
  ...
  if (seleccionTipoSoftware == 0) {
    if (
      softwareRncProveedor == null ||
      softwareRncProveedor == "" ||
      softwareRncProveedor.length < 9 ||
      softwareRncProveedor.length > 11
    ) {
      $("#lblRncProveedorSoftware").attr("hidden", false);
      $("#AlertarSoftwareErrores").attr("hidden", false);
      isrequerido = true;
    }
    if (softwareRazonSocial == null || softwareRazonSocial == "") {
      $("#lblRazonSocialProveedorSoftware").attr("hidden", false);
      $("#AlertarSoftwareErrores").attr("hidden", false);
      isrequerido = true;
    }
